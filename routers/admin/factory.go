...
type FactoryAction int

var factoryAction FactoryAction

func (*FactoryAction)CreateApp(c *gin.Context){
	gh:=igin.H(c)
	app:=factory.App{}
...
func (*FactoryAction)Apps(c *gin.Context){
	gh:=igin.H(c)
	apps,total:=factory.Listing(gh.Account())
	gh.Succ(gin.H{"items":apps,"total":total})
}

func (*FactoryAction)Get(c *gin.Context){
	gh:=igin.H(c)
...
func (*FactoryAction)Req(c *gin.Context){
